...
// ─── PRINT INVOICE MODAL ──────────────────────────────────────────────────────
function PrintInvoiceModal({ order, businessName, onClose }) {
  const biz = businessName || 'Whats-Order'
  ...
  const print = () => {
    ...
    w.document.write(`
      ...
      <div class="header">
        <div><div class="brand">${biz}</div><div class="brand-sub">Order Management</div></div>
      ...
      <div class="footer">Generated by ${biz} • ${new Date().toLocaleString('en-IN')}</div>
      </body></html>
    `)
    w.document.close()
    w.focus()
      ...
      <div className="modal-box" onClick={e => e.stopPropagation()} style={{ maxWidth:480 }}>
        ...
        <div style={{ background:'#F9FAFB', border:'1px solid #E4EDE6', borderRadius:10, padding:20, marginBottom:20 }}>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:16, paddingBottom:12, borderBottom:'2px solid #0A6640' }}>
            <div style={{ fontWeight:800, color:'#0A6640', fontSize:16 }}>{biz}</div>
...
function OrdersView({ addToast }) {
  ...
  useEffect(() => {
    load()
    api.getMe().then(m => setBusinessName(m?.business_name || m?.name || '')).catch(() => {})
  }, [])
  ...
  return (
    <div className="ord-view">
      ...
      {printModal && <PrintInvoiceModal order={printModal} businessName={businessName} onClose={() => setPrintModal(null)} />}
